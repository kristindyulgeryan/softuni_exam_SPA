...
const endpoints = {
  ...
  own: (motorcycleId, userId) =>
    `/data/motorcycles?where=_id%3D%22${motorcycleId}%22%20and%20_ownerId%3D%22${userId}%22&count`,
};
...
describe("E2E tests", function () {
  ...
  describe("Navigation bar [ 10 Points ]", () => {
    it("Logged user should see correct navigation [ 2.5 Points ]", async function () {
      ...
      //Test for navigation
      await page.waitForSelector("nav >> text=Motorcycles", {
        timeout: interval,
      });
    ...
    it("Guest user should see correct navigation [ 2.5 Points ]", async function () {
      ...
      expect(await page.isVisible("nav"), "Dashboard is not visible").to.be
        .true;
      expect(
        await page.isVisible("nav >> text=Add Motorcycle"),
        "Create is visible"
      ).to.be.false;
    ...
    it("Logged in user navigation should work [ 2.5 Points ]", async function () {
      ...
      await page.waitForSelector("#dashboard", { timeout: interval });
      let createBtn = await page.waitForSelector("nav >> text=Add Motorcycle", {
        timeout: interval,
      });
  ...
  describe("Dashboard Page [ 15 Points ]", function () {
    it("Show Motorcycles page - welcome message [ 2.5 Points ]", async function () {
      ...
      await page.waitForSelector("main >> text=Motorcycles", {
        timeout: interval,
      });
    ...
    it("Check Motorcycles page with 0 Motorcycles [ 2.5 Points ]", async function () {
      ...
      await page.waitForSelector("text=No avaliable motorcycles yet.", {
        timeout: interval,
      });
      expect(await page.isVisible("text=No avaliable motorcycles yet.")).to.be
        .true;
    });
  ...
  describe("CRUD [ 50 Points ]", () => {
    describe("Create [ 12.5 Points ]", function () {
      it("Create does NOT work with empty fields [ 2.5 Points ]", async function () {
        ...
        let addFactButton = await page.waitForSelector("text=Add Motorcycle", {
          timeout: interval,
        });
      ...
      it("Create makes correct API call [ 2.5 Points ]", async function () {
        ...
        let modelElement = await page.waitForSelector('[name="model"]', {
          timeout: interval,
        });
        let imageElement = await page.waitForSelector('[name="imageUrl"]', {
          timeout: interval,
        });
        let yearElement = await page.waitForSelector('[name="year"]', {
          timeout: interval,
        });
        let mileageElement = await page.waitForSelector('[name="mileage"]', {
          timeout: interval,
        });
        let contactInfoElement = await page.waitForSelector(
          '[name="contact"]',
          { timeout: interval }
        );
        let aboutElement = await page.waitForSelector('[name="about"]', {
          timeout: interval,
        });
        let submitBtn = await page.waitForSelector('[type="submit"]', {
          timeout: interval,
    ...
    describe("Details [ 10 Points ]", function () {
      it("Details calls the correct API [ 2.5 Points ]", async function () {
        ...
        let moreInfoButton = await page.waitForSelector(
          `.motorcycle:has-text("${data.model}") >> .details-btn`,
          { timeout: interval }
        );
      ...
      it("Details with guest calls shows correct info [ 2.5 Points ]", async function () {
        ...
        let modelElement = await page.waitForSelector("#details-title", {
          timeout: interval,
        });
        let yearElement = await page.waitForSelector(".year", {
          timeout: interval,
        });
        let mileageElement = await page.waitForSelector(".mileage", {
          timeout: interval,
        });
        let contactElement = await page.waitForSelector(".contact", {
          timeout: interval,
        });
        let aboutElement = await page.waitForSelector(
    ...
    describe("Edit [ 17.5 Points ]", function () {
      ...
      it("Edit does NOT work with empty fields [ 2.5 Points ]", async function () {
        ...
        let modelInput = await page.waitForSelector('[name="model"]', {
          timeout: interval,
        });
        let imageInput = await page.waitForSelector('[name="imageUrl"]', {
          timeout: interval,
        });
        let yearInput = await page.waitForSelector('[name="year"]', {
          timeout: interval,
        });
        let mileageInput = await page.waitForSelector('[name="mileage"]', {
          timeout: interval,
        });
        let contactInput = await page.waitForSelector('[name="contact"]', {
          timeout: interval,
        });
        let aboutInput = await page.waitForSelector('[name="about"]', {
          timeout: interval,
        });
